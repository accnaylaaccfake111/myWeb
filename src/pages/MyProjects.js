...
// --- CONSTANTS (đã đổi màu sang đỏ/xám) ---
const PROJECT_CONFIG = {
  FACE_SWAP: {
    name: "Ghép Mặt Vui Nhộn",
    description: "Tạo video ghép mặt độc đáo với AI",
    color: "from-red-600 to-red-800",
    bgColor: "bg-gradient-to-br from-red-50 to-gray-100 border-red-200",
    icon: <Smile className="w-5 h-5" />,
    link: "/face-swap",
  },
  DANCE_SIMULATION: {
    name: "Mô Phỏng Động Tác",
    description: "Mô phỏng và đánh giá động tác",
    color: "from-red-600 to-red-800",
    bgColor: "bg-gradient-to-br from-red-50 to-gray-100 border-red-200",
    icon: <Activity className="w-5 h-5" />,
    link: "/dancing-simulation",
...
const STATUS_CONFIG = {
  DRAFT: { text: "Bản nháp", color: "bg-gray-100 text-gray-700 border-gray-200", icon: <File className="w-3 h-3" /> },
  PROCESSING: { text: "Đang xử lý", color: "bg-blue-50 text-blue-700 border-blue-200", icon: <RefreshCw className="w-3 h-3 animate-spin" /> },
  COMPLETED: { text: "Hoàn thành", color: "bg-green-50 text-green-700 border-green-200", icon: <Check className="w-3 h-3" /> },
  FAILED: { text: "Thất bại", color: "bg-red-50 text-red-700 border-red-200", icon: <AlertTriangle className="w-3 h-3" /> },
  SHEET_COMPLETED: { text: "Hoàn thành", color: "bg-green-50 text-green-700 border-green-200", icon: <Check className="w-3 h-3" /> },
};
...
const MyProjects = ({ isLoggedIn }) => {
  ...
  const getStatusConfig = useCallback((status) => STATUS_CONFIG[status] || { text: status, color: "bg-gray-100 text-gray-700 border-gray-200", icon: <File className="w-3 h-3" /> }, []);
  ...
  const renderProjectPreview = useCallback((project, projectType) => {
    ...
    switch (projectType) {
      ...
      case "CLOTH_SWAP":
        return project.resultImage ? (
          <img src={project.resultImage} alt="Kết quả" className="w-full h-full object-cover" />
        ) : <div className="w-full h-full flex items-center justify-center bg-red-50"><Shirt className="w-8 h-8 text-red-500" /></div>;

      case "DANCE_SIMULATION":
        return <div className="w-full h-full flex items-center justify-center bg-red-50"><Activity className="w-8 h-8 text-red-400" /></div>;

      case "SHEET_MUSIC":
        return <div className="w-full h-full flex items-center justify-center bg-red-50"><Music className="w-8 h-8 text-red-500" /></div>;
  ...
  return (
    <div className="min-h-full bg-gradient-to-b from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          <h1 className="py-3 text-4xl md:text-5xl font-bold bg-gradient-to-r from-red-700 to-red-800 bg-clip-text text-transparent mb-4">
            Dự Án Của Tôi
          </h1>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Quản lý và theo dõi tất cả dự án sáng tạo của bạn
          </p>
        </div>

        {/* Tabs - đã đổi màu sang đỏ và căn giữa */}
        <div className="mb-6 flex flex-wrap justify-center gap-4">
          {projectTypes.map((type) => (
            <button
              key={type}
              onClick={() => setActiveTab(type)}
              className={`flex justify-center items-center space-x-3 px-8 py-3 rounded-xl font-semibold transition-all duration-300 w-full sm:w-auto ${
                activeTab === type 
                  ? "bg-gradient-to-r from-red-700 to-red-800 text-white shadow-lg scale-105" 
                  : "bg-white text-gray-700 hover:bg-gray-50 border border-gray-200"
              }`}
            >
              {PROJECT_CONFIG[type].icon}
              <span className="inline">{PROJECT_CONFIG[type].name}</span>
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6 p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
            />
          </div>
          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-red-500"
          >
            <option value="all">Tất cả trạng thái</option>
            <option value="COMPLETED">Hoàn thành</option>
            <option value="PROCESSING">Đang xử lý</option>
          ...
          <div className="flex bg-gray-100 rounded-lg p-1">
            <button
              ...
              className={`p-2 rounded ${viewMode === "grid" ? "bg-white text-red-600 shadow" : "text-gray-500"}`}
            >
              <Grid className="w-4 h-4" />
            </button>
            <button
              onClick={() => setViewMode("list")}
              className={`p-2 rounded ${viewMode === "list" ? "bg-white text-red-600 shadow" : "text-gray-500"}`}
            >
        ...
        {!isLoggedIn ? (
          <div className="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <Link to="/login" className="inline-block bg-red-700 text-white px-6 py-2 rounded-lg hover:bg-red-800 transition">
              Đăng nhập ngay
            </Link>
          </div>
        ) : currentLoading ? (
          ...
          <div className="text-center py-12">
            ...
            >
              <Plus className="w-4 h-4 mr-1" /> Tạo dự án mới
            </Link>
          </div>
        ) : (
          <div className={viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "space-y-4"}>
            {filteredProjects.map((project) => (
              <div
                key={project.id}
                className={`bg-white rounded-2xl p-5 border border-gray-200 hover:border-red-300 transition-all duration-300 hover:shadow-lg ${
                  viewMode === "list" ? "flex items-center gap-4" : ""
                }`}
              >
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-2">
                    <div className={`p-3 rounded-xl bg-gradient-to-r from-red-700 to-red-800 text-white`}>
                      {PROJECT_CONFIG[activeTab].icon}
                    </div>
                    <div className={`text-xs px-2 py-1 rounded-full border ${getStatusConfig(project.status).color}`}>
                      {getStatusConfig(project.status).text}
                    </div>
                  </div>
                  <h3 className="font-bold text-gray-800 line-clamp-2 mb-2">{getDisplayName(project, activeTab)}</h3>
                  {viewMode === "grid" && (
                    <div className="aspect-video bg-gray-50 rounded-lg overflow-hidden mb-3 border border-gray-200">
                      {renderProjectPreview(project, activeTab)}
                    </div>
                  )}
                  <div className="text-xs text-gray-500 flex items-center gap-2">
                    <Calendar className="w-3 h-3" /> {formatDate(project.createdAt || project.createAt)}
                  </div>
                </div>
                <div className={viewMode === "list" ? "flex gap-2" : "flex justify-between mt-4 pt-3 border-t border-gray-100"}>
                  <button
                    onClick={() => handleView(project)}
                    className="text-red-600 text-sm font-medium flex items-center gap-1 hover:text-red-700 transition"
                  >
                    <Eye className="w-4 h-4" /> Xem
                  </button>
                  <button
                    onClick={() => handleDeleteProject(project, activeTab)}
        ...
        {/* --- MODAL CHI TIẾT (đã đổi màu sang đỏ/xám) --- */}
        {modalContent && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-6">
                ...
                {modalContent.isFaceSwap && (
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      <h4 className="font-semibold text-gray-800 mb-2">Video Kết Quả</h4>
                      {modalContent.resultUrl ? (
                        <video src={modalContent.resultUrl} controls className="w-full rounded-lg shadow-sm" />
                      ) : (
                        <div className="h-48 bg-gray-100 rounded flex items-center justify-center text-gray-500">Chưa có kết quả</div>
                      )}
                    </div>
                    <div className="space-y-4">
                      <div className="bg-red-50 p-4 rounded-lg border border-red-100">
                        <div className="text-sm text-red-800">Trạng thái: <strong>{getStatusConfig(modalContent.status).text}</strong></div>
                        <div className="text-sm text-red-800 mt-1">Thời gian xử lý: <strong>{Math.round((modalContent.processingTimeMs || 0) / 1000)}s</strong></div>
                        <div className="text-sm text-red-800 mt-1">Model: <strong>{modalContent.aiModelVersion}</strong></div>
                      </div>
                      {modalContent.targetUrl && (
                        <div>
                          <h4 className="text-sm font-medium text-gray-700 mb-1">Video Gốc:</h4>
                          <video src={modalContent.targetUrl} className="w-full h-32 object-cover rounded border" controls />
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {modalContent.isSheetMusic && (
                  <div>
                    <div className="bg-red-50 p-4 rounded-lg mb-4 flex flex-wrap gap-4 text-sm border border-red-100">
                      <div className="text-red-800">Chủ đề: <strong>{modalContent.theme}</strong></div>
                      <div className="text-red-800">Tâm trạng: <strong>{modalContent.mood}</strong></div>
                      <div className="text-red-800">Thời lượng: <strong>{modalContent.duration}s</strong></div>
                    </div>
                    {modalContent.sheetMusicXML ? (
                      <div className="border rounded-lg p-4 bg-white shadow-inner">
                        <MusicXMLViewer src={modalContent.sheetMusicXML} />
                    ...
                    {modalContent.lyrics && (
                      <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <h4 className="font-bold text-gray-800 mb-2">Lời bài hát:</h4>
                        <p className="whitespace-pre-wrap text-gray-700">{modalContent.lyrics}</p>
                      </div>
                    )}
                    {modalContent.audioFile && <audio controls src={modalContent.audioFile} className="w-full mt-4" />}
                ...
                {/* 3. DANCE SIMULATION MODAL */}
                {modalContent.isDanceSimulation && (
                  <div>
                    <div className="bg-red-50 p-4 rounded-lg mb-4 grid grid-cols-2 gap-4 text-sm border border-red-100">
                      <div className="text-red-800">Loại: <strong>{modalContent.type}</strong></div>
                      <div className="text-red-800">
                        Điểm số: <strong>{modalContent.json?.average_similarity_score ? Math.round(modalContent.json.average_similarity_score * 100) : 0}%</strong>
                      </div>
                    </div>
                    <div className="h-[400px] w-full bg-gray-900 rounded-xl overflow-hidden relative">
                      {modalContent.type === "PROCESS_VIDEO_STREAM" && modalContent.json ? (
                ...
                {modalContent.isClothSwap && (
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <h4 className="text-sm font-semibold text-gray-700 mb-2 text-center">Trang phục</h4>
                      <img src={modalContent.garmentImage} alt="" className="w-full rounded-lg border border-gray-200 aspect-[3/4] object-cover" />
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold text-gray-700 mb-2 text-center">Người mẫu</h4>
                      <img src={modalContent.modelImage} alt="" className="w-full rounded-lg border border-gray-200 aspect-[3/4] object-cover" />
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold text-gray-700 mb-2 text-center text-red-600">Kết quả</h4>
                      <img src={modalContent.resultImage} alt="" className="w-full rounded-lg border-2 border-red-500 aspect-[3/4] object-cover" />
                    </div>
                  </div>
                )}
              </div>
        ...
        {deleteModalOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-xl max-w-sm w-full text-center">
              ...
              <h3 className="font-bold text-lg mb-2 text-gray-800">Xác nhận xóa?</h3>
              <p className="text-gray-500 mb-6">Hành động này không thể hoàn tác.</p>
              {deleteError && <p className="text-red-500 text-sm mb-4">{deleteError}</p>}
              <div className="flex gap-3 justify-center">
                <button onClick={cancelDelete} className="px-4 py-2 bg-gray-100 rounded-lg font-medium text-gray-700 hover:bg-gray-200 transition">
                  Hủy
                </button>
                <button onClick={confirmDelete} disabled={deleteLoading} className="px-4 py-2 bg-red-600 text-white rounded-lg font-medium flex items-center hover:bg-red-700 transition">
                  {deleteLoading && <RefreshCw className="w-4 h-4 animate-spin mr-2" />} Xóa
                </button>
              </div>
            </div>
          </div>
