    ...
    async (project, type) => {
      try {
        ...
        switch (type) {
          ...
          case "DANCE_SIMULATION":
            detail = await projectService.fetchDanceSimulationDetail(
              project.id
            );
  ...
  const handleView = useCallback(
    async (project) => {
      ...
      try {
        ...
        setError(
          `Lỗi khi tải chi tiết ${PROJECT_CONFIG[activeTab].name.toLowerCase()}`
        );
      }
    },
          ...
          >
            ...
            <div className="flex items-center justify-between text-sm text-gray-500 mb-3">
              {/* <div className="flex items-center space-x-1">
                ...
                <span>{formatDate(project.createdAt || project.createAt)}</span>
              </div> */}
  ...
  const renderEmptyState = () => (
    <div className="text-center py-12">
      ...
      <p className="text-gray-500 mb-6 max-w-md mx-auto">
        {PROJECT_CONFIG[activeTab].description}. Hãy bắt đầu tạo dự án đầu tiên
        của bạn!
      </p>
      <Link
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Quản lý và theo dõi tất cả dự án sáng tạo của bạn tại một nơi
          </p>
        </div>
            ...
            <div className="max-w-md w-full text-center">
              <div className="p-8">
                ...
                <p className="text-gray-600 mb-6">
                  Đăng nhập để xem và quản lý các dự án của bạn
                </p>
                <Link
                  to="/login"
            ...
            <div className="bg-white rounded-2xl p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                {modalContent.isFaceSwap && (
                  <div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                      ...
                      {modalContent.processingTimeMs && (
                        <div>
                          <span className="text-gray-500">
                            Thời gian xử lý:
                          </span>
                          <span className="ml-2 font-medium">
                            {Math.round(modalContent.processingTimeMs / 1000)}s
                          </span>
                        </div>
                      )}
                    </div>
                ...
                {modalContent.isSheetMusic && (
                  <div>
                    {modalContent.content ? (
                      <MusicXMLViewer musicXML={modalContent.content} />
                    ) : (
                      <div className="text-gray-500 text-center py-8">
                        Chưa có nội dung nhạc
                      </div>
                    )}
                  </div>
                )}
                ...
                {modalContent.isDanceSimulation && (
                  <div>
                    {modalContent.skeletonData ? (
                      <SkeletonViewer
                        skeletonData={modalContent.skeletonData}
                      />
                    ) : (
                      <div className="text-gray-500 text-center py-8">
                ...
                {modalContent.isClothSwap && (
                  <div>
                    {modalContent.resultImage ? (
                      <img
                        src={modalContent.resultImage}
                        alt="Kết quả"
                        className="w-full rounded-lg"
                      />
                    ) : (
                      <div className="text-gray-500 text-center py-8">
                        Chưa có hình ảnh kết quả
                      </div>
                    )}
                    <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                      <div>
                      ...
                      {modalContent.timeProcessing && (
                        <div>
                          <span className="text-gray-500">
                            Thời gian xử lý:
                          </span>
                          <span className="ml-2 font-medium">
                            {Math.round(modalContent.timeProcessing / 1000)}s
                          </span>
                        </div>
                      )}
                    </div>
