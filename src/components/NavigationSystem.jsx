...
const NavigationSystem = ({ isLoggedIn, user, onLogout }) => {
    ...
    const mainNavItems = [
        { path: "/", label: "Khám phá", icon: <Home className="w-5 h-5" />, description: "Trang chủ" },
        {path: "/cultural-history", label: "Thư viện", icon: <BookOpen className="w-5 h-5" />, description: "Tìm hiểu lịch sử",},
        { path: "/my-projects", label: "Dự án của tôi", icon: <FolderOpen className="w-5 h-5" />, description: "Công việc đã lưu" },
        { path: "/feedback", label: "Đánh giá", icon: <Star className="w-5 h-5" />, description: "Chia sẻ ý kiến" },
    ];

    const isActivePath = (path) => location.pathname === path || location.pathname.startsWith(path + "/");

    const DropdownMenu = () => (
        <div className="absolute right-0 mt-2 w-64 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-red-100 py-3 z-50">
            {isLoggedIn ? (
                <>
                    <div className="px-4 py-3 border-b border-red-50">
                        <div className="flex items-center space-x-3">
                            <div className="h-10 w-10 rounded-full bg-gradient-to-br from-red-600 to-red-800 flex items-center justify-center text-white font-semibold text-sm">
                                {avatarInitial}
                            </div>
                            <div>
                                <p className="font-semibold text-gray-800 text-sm">{displayName}</p>
                                <p className="text-xs text-gray-500 mt-0.5">{user?.email || "Thành viên"}</p>
                            </div>
                        </div>
                    </div>
                    <div className="space-y-1 px-2 py-2">
                        <button onClick={() => { setShowDropdown(false); navigate("/profile"); }} className="flex items-center space-x-3 w-full px-3 py-2.5 rounded-xl text-gray-700 hover:bg-red-50 transition-all duration-200 group">
                            <User className="h-4 w-4 text-gray-500 group-hover:text-red-500" />
                            <span className="font-medium text-sm">Hồ sơ cá nhân</span>
                        </button>
                        <button onClick={() => { setShowDropdown(false); navigate("/my-projects"); }} className="flex items-center space-x-3 w-full px-3 py-2.5 rounded-xl text-gray-700 hover:bg-red-50 transition-all duration-200 group">
                            <FolderOpen className="h-4 w-4 text-gray-500 group-hover:text-red-500" />
                            <span className="font-medium text-sm">Dự án của tôi</span>
                        </button>
                    </div>
                    <div className="border-t border-red-50 mt-2 pt-2 px-2">
                        <button onClick={() => { setShowDropdown(false); onLogout(); }} className="flex items-center space-x-3 w-full px-3 py-2.5 text-gray-500 hover:bg-gray-50 transition-all duration-200 rounded-xl group">
                            <LogOut className="h-4 w-4 group-hover:text-red-500" />
                ...
                <div className="space-y-3 p-3">
                    <p className="text-sm text-gray-600 text-center mb-2">Tham gia cộng đồng sáng tạo</p>
                    <button onClick={() => { setShowDropdown(false); navigate("/login"); }} className="flex items-center justify-center w-full px-4 py-3 bg-gradient-to-r from-red-600 to-red-800 text-white rounded-xl font-semibold shadow-md hover:shadow-lg transition-all duration-300 text-sm">
                        <User className="h-4 w-4 mr-2" /> Đăng nhập ngay
                    </button>
                    <button onClick={() => { setShowDropdown(false); navigate("/register"); }} className="flex items-center justify-center w-full px-4 py-3 border-2 border-red-300 text-red-700 rounded-xl font-semibold hover:bg-red-50 transition-all duration-300 text-sm">
                        Tạo tài khoản mới
                    </button>
                </div>
            )}
    ...
    return (
        <div className="min-h-dvh bg-gradient-to-br from-gray-50 to-gray-100">
            <header className={`sticky top-0 z-40 transition-all duration-500 ${
                isScrolled ? "bg-white/80 backdrop-blur-xl border-b border-red-100 shadow-md" : "bg-white/60 backdrop-blur-lg border-b border-red-50"
            }`}>
                <div className="max-w-7xl mx-auto">
                    <div className="flex items-center justify-between h-20">
                        <div className="flex items-center">
                            <button onClick={() => setShowMobileSidebar(!showMobileSidebar)} className="p-2 rounded-xl hover:bg-red-100 transition-all duration-300 lg:hidden">
                                <Menu className="h-5 w-5 text-red-600" />
                            </button>
                            <Link to="/" className="flex items-center space-x-3 group">
                                <div className="relative">
                                    <div className="h-10 w-10 rounded-2xl bg-gradient-to-br from-red-600 to-red-800 flex items-center justify-center text-white shadow-md group-hover:shadow-lg transition-all duration-300 group-hover:scale-105">
                                        <Sparkles className="h-5 w-5" />
                                    </div>
                                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-white animate-pulse"></div>
                                </div>
                                <div className="hidden sm:block">
                                    <span className="text-xl font-bold text-gray-800">KATA</span>
                                </div>
                            </Link>
                        </div>
                        <div className="flex items-center space-x-4">
                            <div className="relative" ref={dropdownRef}>
                                <button onClick={() => setShowDropdown(!showDropdown)} className="flex items-center space-x-3 bg-white/80 backdrop-blur-sm pl-3 pr-3 py-2 rounded-2xl shadow-sm border border-red-100 hover:shadow-md hover:border-red-200 transition-all duration-300 group">
                                    <div className="relative">
                                        <div className="h-8 w-8 rounded-full bg-gradient-to-br from-red-600 to-red-800 flex items-center justify-center text-white font-semibold text-sm group-hover:scale-110 transition-transform duration-300">
                                            {isLoggedIn ? avatarInitial : <User className="h-4 w-4" />}
                                        </div>
                                    </div>
                                    <div className="hidden sm:block text-left">
                                        <span className="font-semibold text-gray-800 text-sm block">{isLoggedIn ? displayName : "Xin chào!"}</span>
                                        <span className="text-xs text-gray-500">{isLoggedIn ? "Tài khoản" : "Đăng nhập ngay"}</span>
                                    </div>
            ...
            <div className="flex max-w-7xl mx-auto">
                {/* Desktop Sidebar - sáng, trong suốt */}
                <aside className="sticky top-20 hidden lg:block w-80 h-fit flex-shrink-0 py-8">
                    <div className="bg-white/70 backdrop-blur-md rounded-3xl shadow-md border border-red-100/60 p-6 sticky top-24">
                        <nav className="mb-8">
                            <div className="space-y-2">
                                {mainNavItems.map((item) => {
                                    const isActive = isActivePath(item.path);
                                    return (
                                        <Link key={item.path} to={item.path} className={`flex items-center space-x-4 p-4 rounded-2xl transition-all duration-300 group ${
                                            isActive ? "bg-gradient-to-r from-red-600 to-red-800 text-white shadow-md" : "text-gray-700 hover:bg-red-50 hover:shadow-sm"
                                        }`}>
                                            <div className={`p-2 rounded-xl transition-colors ${isActive ? "bg-white/20" : "bg-red-100 text-red-600 group-hover:bg-red-200"}`}>
                                                {item.icon}
                                            </div>
                                            <div className="flex-1">
                        ...
                        <div className="mb-8">
                            <h4 className="font-bold text-gray-800 mb-4 flex items-center">
                                <Sparkles className="h-4 w-4 text-red-500 mr-2" /> Công cụ sáng tạo
                            </h4>
                            <div className="space-y-3">
                                {creativeTools.map((tool) => {
                                const isActive = isActivePath(tool.path);
                                return (
                                    <Link 
                                        ...
                                        className={`block p-4 rounded-2xl border transition-all duration-300 ${
                                            isActive 
                                                ? `bg-gradient-to-r ${tool.color} text-white shadow-md border-transparent` 
                ...
                {/* Mobile Sidebar - sáng */}
                {showMobileSidebar && <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-40 lg:hidden" onClick={() => setShowMobileSidebar(false)} />}
                <aside className={`fixed inset-y-0 left-0 transform ${showMobileSidebar ? "translate-x-0" : "-translate-x-full"} lg:hidden transition-transform duration-300 z-50 w-80 bg-white/95 backdrop-blur-xl border-r border-red-100 shadow-xl overflow-y-auto`}>
                    <div className="p-6">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    <h2 className="font-bold text-gray-800">KATA</h2>
                                    <p className="text-xs text-gray-500">Khám phá văn hóa</p>
                                </div>
                            </div>
                            <button onClick={() => setShowMobileSidebar(false)} className="p-2 rounded-xl hover:bg-red-100">
                                <X className="h-5 w-5 text-gray-600" />
                            </button>
                        </div>
                        <nav className="space-y-2 mb-6">
                            {mainNavItems.map((item) => (
                                <Link key={item.path} to={item.path} onClick={() => setShowMobileSidebar(false)} className="flex items-center space-x-3 p-4 rounded-2xl bg-red-50 hover:bg-red-100 transition-colors">
                                    <div className="p-2 rounded-xl bg-white text-red-600">{item.icon}</div>
                                    <div>
                        ...
                        <div className="space-y-3">
                            <h4 className="font-semibold text-gray-800 text-sm">Công cụ sáng tạo</h4>
                            {creativeTools.map((tool) => (
                                <Link key={tool.id} to={tool.path} onClick={() => setShowMobileSidebar(false)} className="block p-4 rounded-2xl bg-white border border-red-100 hover:shadow-md transition-all">
                                    <div className="flex items-center space-x-3">
                                        <div className="p-2 rounded-xl bg-red-50 text-red-600">{tool.icon}</div>
                                        <div>
